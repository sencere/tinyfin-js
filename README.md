...
- `examples/btc-trading.html`: BTC trading simulation using `dataset/btc_4h.csv`.
- `examples/btc-trading-model.js`: generated browser model for the BTC trading backend policy.
- `examples/btc-trading-model.json`: generated BTC model data for tools or inspection.
...
Open `examples/btc-trading.html`.

This demo loads `dataset/btc_4h.csv`, builds normalized rolling OHLCV states, and trains a toy DQN with three actions: hold, long, and flat. It simulates fees, plots BTC close against the agent equity curve, marks trades, and compares against buy-and-hold.

If the browser blocks local CSV loading from `file://`, either use the file picker in the page or serve the project with a local HTTP server.

The same page can also run the native backend model generated by `backend/btc_trainer.c`. Click `Run backend model` to load `examples/btc-trading-model.js`. This backend model is a future-aware offline baseline over `(time, position)` states, so treat it as a comparison/oracle rather than a deployable trading system.
...
- `examples/btc-trading-model.js`
- `examples/btc-trading-model.json`
...
To regenerate only the BTC trading backend model:
